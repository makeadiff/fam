...
$all_connections = $sql->getById("SELECT user_id, evaluator_id FROM FAM_UserEvaluator WHERE evaluator_id != 0 AND group_id=$group_id AND year=$year");

// Remove applicants rejected during the last stage.
if($active_stage_id > 1) {
	$last_stage_id = $active_stage_id - 1;
	$rejected_applicants = $sql->getCol("SELECT user_id FROM FAM_UserStage WHERE stage_id<=$last_stage_id AND status='rejected' AND year=$year");
...
$data_entered = [];
if($active_category_id and count($cache_user_ids)) {
	...
	if(count($parameters_in_category)) {
		$data_entered = $sql->getAll("SELECT evaluator_id, COUNT(DISTINCT user_id) AS users 
			FROM FAM_Evaluation 
			WHERE year={$year} AND parameter_id IN (" . implode(",", $parameters_in_category) . ") AND evaluator_id IN (" . implode(',', $cache_user_ids) . ")
